...
// ============================================================
const q_pedidos = query(refPedidos, orderBy("fecha", "desc"), limit(50));
...
document.getElementById("form-pedido").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    await runTransaction(db, async (tx) => {
      ...
      // 5) Guardamos el pedido
      const total = lineas.reduce((acc, l) => acc + l.producto.precio * l.cantidad, 0);
      const items = lineas.map(l => ({ productoId: l.productoId, nombre: l.producto.nombre, cantidad: l.cantidad, precioUnit: l.producto.precio }));
      tx.set(doc(refPedidos), { fecha: serverTimestamp(), items, total });
    });
