...
function pintarPedidos(){
  ...
  // Los pedidos normales aparecen siempre aquí. Las reservas solo
  // aparecen aquí una vez ya fueron entregadas (mientras están
  // pendientes, viven en la sección de Reservas de arriba).
  const visibles = pedidosCache.filter(p => !(p.tipo === "reserva" && p.estado !== "entregado"));
  ...
  cont.innerHTML = `
    <table>
      <thead><tr><th>Fecha</th><th>Items</th><th>Total</th><th>Estado</th><th></th></tr></thead>
      <tbody>
        ${visibles.map(p => `
          <tr>
            ...
            <td><span class="badge ${p.estado === 'entregado' ? 'ok' : 'bajo'}">${p.estado === 'entregado' ? 'Entregado' : 'Pendiente'}</span></td>
            <td>${p.estado !== 'entregado' ? `<button class="btn secundario" style="padding:5px 9px;font-size:12px" data-marcar-entregado="${p.id}">Marcar entregado</button>` : ''}</td>
          </tr>
        `).join("")}
      </tbody>
    </table>`;

  cont.querySelectorAll("[data-marcar-entregado]").forEach(b =>
    b.addEventListener("click", () => marcarEntregado(b.dataset.marcarEntregado)));
}

// Solo reservas que aún no se han entregado — ordenadas por hora de entrega.
function pintarReservas(){
  const cont = document.getElementById("lista-reservas");
  const reservas = pedidosCache
    .filter(p => p.tipo === "reserva" && p.estado !== "entregado")
    .sort((a,b) => (a.horaEntrega || "").localeCompare(b.horaEntrega || ""));

  if (reservas.length === 0){
    cont.innerHTML = `<p class="vacio">No hay reservas pendientes.</p>`;
    return;
  }
  cont.innerHTML = reservas.map(p => `
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">
        <div>
          <div class="nombre" style="font-weight:600">
            ${p.horaEntrega ? `Entregar a las ${p.horaEntrega}` : "Sin hora definida"}
          </div>
          <div style="color:var(--text-dim);font-size:12.5px;margin-top:4px">
            ${(p.items||[]).map(it => `${it.cantidad}× ${it.nombre}`).join(", ")} · $${formatear(p.total)}
          </div>
        </div>
        <button class="btn secundario" style="padding:5px 9px;font-size:12px" data-marcar-entregado="${p.id}">Marcar entregado</button>
      </div>
    </div>
  `).join("");
...
function abrirModalPedido(){
  ...
  document.querySelector('input[name="tipoPedido"][value="normal"]').checked = true;
...
document.querySelectorAll('input[name="tipoPedido"]').forEach(r => r.addEventListener("change", actualizarVisibilidadReserva));

function actualizarVisibilidadReserva(){
  const tipo = document.querySelector('input[name="tipoPedido"]:checked').value;
  const campo = document.getElementById("campo-hora-entrega");
  campo.style.display = tipo === "reserva" ? "block" : "none";
  campo.querySelector('input[name="horaEntrega"]').required = tipo === "reserva";
}
...
document.getElementById("form-pedido").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    await runTransaction(db, async (tx) => {
      ...
      const tipo = document.querySelector('input[name="tipoPedido"]:checked').value; // 'normal' o 'reserva'
      const horaEntrega = tipo === "reserva" ? (document.querySelector('input[name="horaEntrega"]').value || null) : null;

      // 6) Guardamos el pedido — siempre arranca en estado "pendiente";
      // desde la pestaña Pedidos/Reservas lo marcas como "entregado" cuando ya salió.
      tx.set(doc(refPedidos), {
        fecha: serverTimestamp(), items, total, costoTotal, gananciaTotal,
        tipo, horaEntrega, estado: "pendiente"
      });
    });
